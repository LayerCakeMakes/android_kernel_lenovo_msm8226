...
#define Battey_99to100_PERIOD  (msecs_to_jiffies(1000*60))
#define Battery_100to99_PERIOD  (msecs_to_jiffies(1000*60))
int battery_check_100=0;
int battery_check_99=0;
static int work_queue_status = 0;
...
struct msm_charger_mux {
	...
	struct delayed_work battery_100_99_work;
...
extern int recharging_fake_soc_flag;
static int get_prop_batt_capacity(void)
{
	int capacity;
	static int capacity_UI;
	...
	//recharging period fake 100
        if(recharging_fake_soc_flag == 1 && capacity > 95 )
        {
                capacity_UI = 100;
                printk(KERN_ERR "%s, recharging fake %d",__func__,capacity_UI);
		return capacity_UI;
        }

	if(bq24196_is_charging == 0 && fake_step > 0)
	{
		printk(KERN_ERR "%s,0 capacity:%d,capacity_UI:%d", __FUNCTION__,capacity,capacity_UI);

		if( battery_check_99 >= 2)
                {
                        printk(KERN_ERR "4%s", __FUNCTION__);
                        capacity_UI--;// = capacity;
						fake_step -- ;
                        cancel_delayed_work_sync(&msm_chg.battery_100_99_work);
						battery_check_99 = 0 ;
                        work_queue_status =0;
                }
                else
                {
                        printk(KERN_ERR "5%s", __FUNCTION__);
                        if(work_queue_status==0)
                        {
                                work_queue_status = 1;
                                queue_delayed_work(msm_chg.event_wq_thread,
                                        &msm_chg.battery_100_99_work,Battery_100to99_PERIOD);
                        }
                }
	...
	//adjust capacity
	if(capacity == 99  &&   bq24196_is_charging == 1 )//charging
	{
		printk(KERN_ERR "1%s", __FUNCTION__);
		if(battery_check_100 > 25)
		{
			printk(KERN_ERR "2%s", __FUNCTION__);
			capacity_UI = 100;
			cancel_delayed_work(&msm_chg.battery_100_99_work);
			work_queue_status =0;
		} 
		else
		{
			capacity_UI = capacity;
			printk(KERN_ERR "3%s", __FUNCTION__);
			if(work_queue_status==0)
			{
				work_queue_status = 1;
				queue_delayed_work(msm_chg.event_wq_thread,
				&msm_chg.battery_100_99_work,Battey_99to100_PERIOD);
			}
		}
	}
	else if(bq24196_is_charging == 0 && capacity_UI == 100 && capacity < 100 )//discharge
	{
		printk(KERN_ERR "%s,1 capacity:%d,capacity_UI:%d", __FUNCTION__,capacity,capacity_UI);
		fake_step  = capacity_UI - capacity ;
	}
	else
	{
		printk(KERN_ERR "6%s", __FUNCTION__);
		cancel_delayed_work(&msm_chg.battery_100_99_work);
		battery_check_99 = 0;
		battery_check_100 = 0;
		work_queue_status =0;
...
static int get_prop_batt_status(void)
{
	...
	printk(KERN_ERR "%s:msm_chg.batt_status = %d \n",__func__, msm_chg.batt_status);
...
static int get_prop_batt_temp(void)
{
	...
	if (msm_batt_gauge && msm_batt_gauge->get_battery_temperature)
	{
		temp = msm_batt_gauge->get_battery_temperature();
	}
	//temp = temp/10;
...
extern int bq24196_get_chrg_current( void );

static int get_prop_batt_current(void)
{
	int current_chg=0;

	if(msm_chg.batt_status == BATT_STATUS_TRKL_CHARGING || msm_chg.batt_status == BATT_STATUS_FAST_CHARGING)
	{
		current_chg = bq24196_get_chrg_current();
	}
	else
	{
		current_chg = 0;
	...
	printk("battery charge current=%d \n", current_chg);
...
static void battery_full_check(struct work_struct *work)
{
	printk(KERN_ERR "bq24196_is_charging =%d,battery_check_100=%d,battery_check_99= %d\n",bq24196_is_charging,battery_check_100,battery_check_99);
...
static void handle_event(struct msm_hardware_charger *hw_chg, int event)
{
	...
	printk(KERN_ERR "bq24196 1%s ,%d,event:%d",__func__, (int)hw_chg ,event);
	...
	case CHG_INSERTED_EVENT:
		...
		if (hw_chg->type == CHG_TYPE_USB) {
			...
			//if (usb_chg_current) {
				priv->max_source_current = 500;//usb_chg_current;
#else
			if (usb_chg_current) {
...
static int __devinit msm_charger_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&msm_chg.battery_100_99_work, battery_full_check);
	...
	queue_delayed_work(msm_chg.event_wq_thread,
				&msm_chg.update_heartbeat_work,
			      round_jiffies_relative(msecs_to_jiffies
						     (msm_chg.update_time)));
	queue_delayed_work(msm_chg.event_wq_thread,
				&msm_chg.battery_100_99_work,
				Battery_100to99_PERIOD);
	mutex_lock(&msm_chg.status_lock);
	handle_battery_inserted();
	mutex_unlock(&msm_chg.status_lock);
	return 0;
