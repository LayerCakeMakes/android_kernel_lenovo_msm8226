...
static struct regulator *regulator1;
static struct regulator *regulator2;
#define VDD_2P8         "8226_l19"
#define VDD_I2C         "8226_lvs1"
...
static ssize_t synaptics_rmi4_interruptclr_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
...
#endif
	__ATTR(interruptclr, (S_IWUSR),
		synaptics_rmi4_show_error,
		synaptics_rmi4_interruptclr_store),
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
 ...
 int touchscreen_enable_from_lcd(int val)
 {
	 struct device *dev;

	 if(!exp_data.rmi4_data) {
		 pr_err("synaptics_dsx_i2c: %s: Failed to set touch suspend or resume, no dev.\n",
		 __func__);
		 return -ENODEV;
	 }
	 dev = &(exp_data.rmi4_data->input_dev->dev);
  ...
  EXPORT_SYMBOL(touchscreen_enable_from_lcd);
		...
		const struct i2c_device_id *dev_id)
{
	...
	regulator1 = regulator_get(&client->dev, VDD_2P8);
        if(IS_ERR(regulator1)){
                printk(KERN_ERR "failed to get tp regulator\n");
                return -EINVAL;
        } else{
                if(regulator_count_voltages(regulator1) > 0){
                        regulator_set_voltage(regulator1, 2700000, 3300000);
                        regulator_set_optimum_mode(regulator1, 15000);
                }else
                        printk(KERN_ERR"--------------set voltage error");
        }
	regulator2 = regulator_get(&client->dev, VDD_I2C);
        if(IS_ERR(regulator2)){
                printk(KERN_ERR "failed to get tp regulator\n");
        } else{
                regulator_count_voltages(regulator2);
                regulator_set_voltage(regulator2, 1800000, 1800000);
                regulator_set_optimum_mode(regulator2, 10000);
        }
	...
	retval = gpio_request(platform_data->irq_gpio, "rmi_irq");
...
static void synaptics_rmi4_late_resume(struct early_suspend *h)
{
	...
	dev_info(&rmi4_data->i2c_client->dev,
		"%s: full_pm_cycle(%d), sensor_sleep(%d)\n",
		__func__, rmi4_data->full_pm_cycle, rmi4_data->sensor_sleep);
...
static int synaptics_rmi4_suspend(struct device *dev)
{
	...
	dev_info(&rmi4_data->i2c_client->dev,
					"%s: sensor_sleep(%d),regulator_en(%d)\n",
...
static int synaptics_rmi4_resume(struct device *dev)
{
	...
	dev_info(&rmi4_data->i2c_client->dev,
		"%s: regulator_en(%d)\n",
		__func__, platform_data->regulator_en);
