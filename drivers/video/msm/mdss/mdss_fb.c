...
extern int mdss_get_cabc_mode(void);
extern int mdss_set_cabc_mode(struct msm_fb_data_type *mfd, int mode);
extern char *mdss_get_lcd_name(void);
...
static ssize_t mdss_show_lcd_dbg(struct device *dev,
                                struct device_attribute *attr, char *buf)
{
	ssize_t ret;

	ret = snprintf(buf, PAGE_SIZE, "cabc:%d\n", mdss_get_cabc_mode());
	return ret;
}

static ssize_t mdss_store_lcd_dbg(struct device *dev,
                                 ...
                                 const char *buf, size_t count)
{
	...
	mdss_set_cabc_mode(mfd, on_off);
	return strlen(buf);
}

static ssize_t mdss_show_lcd_name(struct device *dev,
                                struct device_attribute *attr, char *buf)
{
        ssize_t ret;
...
static ssize_t mdss_show_board_id(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	ssize_t ret;
...
static DEVICE_ATTR(lcd_dbg, S_IWUSR | S_IRUGO, mdss_show_lcd_dbg, mdss_store_lcd_dbg);
static DEVICE_ATTR(lcd_name, S_IWUSR | S_IRUGO, mdss_show_lcd_name, NULL);
static DEVICE_ATTR(board_id, S_IWUSR | S_IRUGO, mdss_show_board_id, NULL);

static struct attribute *mdss_fb_attrs[] = {
	&dev_attr_msm_fb_type.attr,
	&dev_attr_msm_fb_split.attr,
	&dev_attr_show_blank_event.attr,
	&dev_attr_lcd_dbg.attr,
			 ...
			 unsigned long arg)
{
	...
	case MSMFB_SET_CABC_MODE:
		ret = copy_from_user(&cabc_mode, argp , sizeof(cabc_mode));
		if (ret< 0)
			return ret;
                ret = mdss_set_cabc_mode(mfd, cabc_mode);
                break;
