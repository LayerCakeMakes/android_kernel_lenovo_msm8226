...
//lenovo added, only for debug purpose
int32_t lenovo_camera_sensor_i2c_debug = 0;
int32_t lenovo_camera_ov9760_ae_disable = 0;
int32_t lenovo_camera_ov8865_ae_disable = 0;
//lenovo ended
	...
	void __user *argp)
{
	...
	case CFG_WRITE_I2C_ARRAY: {
		...
		//lenovo added
		if (lenovo_camera_sensor_i2c_debug)
			break;
		...
		//lenovo added	
		if (lenovo_camera_ov9760_ae_disable || lenovo_camera_ov8865_ae_disable){
			int i;
			int ae_set_skip =0;
			printk("YAO, check ae setting ,as ae disable is set\n");
			for(i=0; i< conf_array.size;i++){
				printk("YAO, reg=%x, val=%x\n",
					conf_array.reg_setting[i].reg_addr,
					conf_array.reg_setting[i].reg_data);
				if ((conf_array.reg_setting[i].reg_addr >= 0x3500) &&
					(conf_array.reg_setting[i].reg_addr >= 0x3502)){
					ae_set_skip = 1;
					break;
	...
	case CFG_SENSOR_OTP_PROC:{
		if (s_ctrl->sensor_otp_proc){
			rc = s_ctrl->sensor_otp_proc(s_ctrl);
			pr_info(KERN_EMERG"YAO,msm_sensor_ioctl:CFG_SENSOR_OTP_PROC %lu\n",rc);
		}
		break;
	}
	default:
