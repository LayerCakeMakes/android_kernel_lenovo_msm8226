...
#ifdef CONFIG_SND_SOC_HEADSET_FSA8049
/* we overide qualcomm US EU headset detection 
  * Qualcomm replied on the detection for the EU US headset.
  * FSA8049 already contains the detection logic and switch logic, duplicate 
  * detection logic might break the Qualcomm detection one that never trigger
  * the interrupt for headset swap
  */
#define VDD_2P8 "8226_l19"
#define GPIO_CHIP_ENABLE 56
static struct regulator *us_eu_headset_sw_reg = NULL;
					 ...
					 enum wcd9xxx_mbhc_plug_type plug_type)
{
	...
	if((mbhc->current_plug == PLUG_TYPE_NONE) && (plug_type == PLUG_TYPE_GND_MIC_SWAP)){
#ifdef CONFIG_SND_SOC_HEADSET_FSA8049
		pr_info("enable fsa8049 again if gnd and mic is reversed!\n");
		gpio_set_value(GPIO_CHIP_ENABLE, 0);
		usleep_range(50000, 50000);
		gpio_set_value(GPIO_CHIP_ENABLE, 1);
		usleep_range(200000, 200000); /* FIXME: give enough time for FSA8049 to be stablize */
#endif	
...
#ifdef CONFIG_SND_SOC_HEADSET_FSA8049
		pr_info("disable fsa8049\n");
		gpio_set_value(GPIO_CHIP_ENABLE, 0);
		usleep_range(100000, 100000); /* 10 mseconds */
#endif
...
#ifdef CONFIG_SND_SOC_HEADSET_FSA8049
	regulator_enable(us_eu_headset_sw_reg);
	gpio_request(GPIO_CHIP_ENABLE, "fsa8049 en_dis");
	gpio_direction_output(GPIO_CHIP_ENABLE, 0); /* 1 to enable this chip for headset detection */
	gpio_set_value(GPIO_CHIP_ENABLE, 0);
	gpio_requested = 1;
	printk(KERN_DEBUG"enable headset detection reg\n");
#endif
...
#ifdef CONFIG_SND_SOC_HEADSET_FSA8049
	regulator_disable(us_eu_headset_sw_reg);
	gpio_free(GPIO_CHIP_ENABLE);
	gpio_requested = 0;
	printk(KERN_DEBUG"disable headset detection reg\n");
#endif
}
...
#ifdef CONFIG_SND_SOC_HEADSET_FSA8049
	us_eu_headset_sw_reg = regulator_get(codec->dev->parent, VDD_2P8);
	if (IS_ERR(us_eu_headset_sw_reg)){
		printk(KERN_ERR "failed to get tp regulator\n");
		ret = -EINVAL;
	} else{
		if(regulator_count_voltages(us_eu_headset_sw_reg) > 0){
			regulator_set_voltage(us_eu_headset_sw_reg, 2700000, 3300000);
			regulator_set_optimum_mode(us_eu_headset_sw_reg, 15000);
		}else {
			printk(KERN_ERR"set voltage error");
			}
