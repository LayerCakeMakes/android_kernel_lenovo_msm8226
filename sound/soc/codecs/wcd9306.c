...
#define YDA_GPIO_PA_MOD_1 31
#define YDA_GPIO_PA_MOD_2 21
...
static int spkr_yda_pa_configure(int mode)
{
	static int requested = 0;
	
	printk(KERN_DEBUG "YDA PA mode %d", mode);

	if (!requested) {
		int ret;
		ret = gpio_request(YDA_GPIO_PA_MOD_1, "spkr_yda_mod_1");
		if (ret < 0) {
			printk(KERN_ERR"mode 1 GPIO request failed\n"); 
			goto out;
			}
		
		ret = gpio_request(YDA_GPIO_PA_MOD_2, "spkr_yda_mod_2");
		if (ret < 0) {
			gpio_free(YDA_GPIO_PA_MOD_1); 
			printk(KERN_ERR"mode 2 GPIO request failed\n"); 
			goto out;
			}
	...
	switch (mode) {
		case 1:
			/*non clip 1 mode */
			gpio_direction_output(YDA_GPIO_PA_MOD_1, 1);
			gpio_direction_output(YDA_GPIO_PA_MOD_2, 1);
		break;
		case 2:
			/* non clip 2 mode */
			gpio_direction_output(YDA_GPIO_PA_MOD_1, 1);
			gpio_direction_output(YDA_GPIO_PA_MOD_2, 0);
		break;
		case 3:
			/* non clip off mode */
			gpio_direction_output(YDA_GPIO_PA_MOD_1, 0);
			gpio_direction_output(YDA_GPIO_PA_MOD_2, 1);
		break;
		case 0:
		default:
