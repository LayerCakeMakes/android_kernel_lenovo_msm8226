...
void msm_q6_enable_mi2s_clocks(void)
{
	union afe_port_config port_config;
	u16 port_id = 0;
	int rc;
	extern int msm_q6_control_ter_mi2s_clocks(int en);

	msm_q6_control_ter_mi2s_clocks(1);
	...
	if (msm_mi2s_get_port_id(MSM_TERT_MI2S, 
...
void msm_q6_disable_mi2s_clocks(void)
{
	u16 port_id = 0;
	int rc;
	extern int msm_q6_control_ter_mi2s_clocks(int en);

	msm_q6_control_ter_mi2s_clocks(0);
...
EXPORT_SYMBOL(msm_q6_disable_mi2s_clocks);


static int last_was_voice_call = 0;
static atomic_t voice_call_session = ATOMIC_INIT(0);
void msm_q6_tfa9887_mono_stereo_control(void);

extern int Tfa9887_get_Channels(void);
void msm_q6_voice_call_session(int active)
{
	pr_debug("%s: active %d\n", __func__, active);

	if(Tfa9887_get_Channels()==2){
		if (active) {
			atomic_inc_return(&voice_call_session);
		}
		else {
			atomic_dec_return(&voice_call_session);
		}
		msm_q6_tfa9887_mono_stereo_control();
	}
}
EXPORT_SYMBOL(msm_q6_voice_call_session);


void msm_q6_tfa9887_mono_stereo_control(void)
{
	extern int Tfa9887_Mono_Stereo_Switch(int mono);
	if (atomic_read(&voice_call_session) > 0) {
		pr_debug("%s, voice call session, set tfa9887 as MONO\n", __func__);

		if (!last_was_voice_call) {
			pr_debug("last was NOT voice call session, switch to MONO\n");
			Tfa9887_Powerdown(1);
			Tfa9887_Powerdown(0); /* warm reboot to make internal dsp standby */
			Tfa9887_Mono_Stereo_Switch(1);
			Tfa9887_Powerdown(1);			
			Tfa9887_Powerdown(0); /* warm reboot to enable function */
			}

		last_was_voice_call = 1;
		}
	else {
		pr_debug("%s, stereo play session\n", __func__);
		if (last_was_voice_call) {
			pr_debug("last was voice call session, switch to STEREO\n");
			Tfa9887_Powerdown(1);
			Tfa9887_Powerdown(0);
			Tfa9887_Mono_Stereo_Switch(0);
			Tfa9887_Powerdown(1);			
			Tfa9887_Powerdown(0);
			}
		last_was_voice_call = 0;
		}
}
				...
				struct snd_soc_dai *dai)
{
	...
	if (tfa9887_init == 0)	{
		...
		/* clock is already supplied to the port */
		Tfa9887_Init(dai_data->rate, 0);
		tfa9887_init = 1;
		/* shutdown clock */
		afe_close(port_id);
	}
