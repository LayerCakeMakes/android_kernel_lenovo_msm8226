...
static int mdss_mdp_parse_bootarg(struct platform_device *pdev)
{
	 ...
	 * check board here 
	 */
	name_len = strlen("board_id=");
	cmd_len = strlen(cmd_line);
	disp_idx = strnstr(cmd_line, "board_id=", cmd_len);
	if (disp_idx > 0 )
	{
		disp_idx += name_len;
		end_idx = strnstr(disp_idx, " ", 8);
		if (end_idx) {
			*end_idx = 0;
			g_board_id = simple_strtol(disp_idx, NULL, 10);
			pr_info("%s-- %s:%d\n",__func__, disp_idx, g_board_id);
			*end_idx = ' ';
		}
	}
