...
static bool is_efuse_mode = false;
#endif
...
{
       int state = __raw_readl(dload_mode_addr+sizeof(unsigned int)*9);
       printk("jarrett get_efuse_state: %d\n",state);
       return state;
}
...
{
      if (dload_mode_addr) {
		__raw_writel(0xE47B33EF , dload_mode_addr);
		__raw_writel(0xCE1409EF ,
		       dload_mode_addr + sizeof(unsigned int));
	    mb();
            dload_mode_enabled = 1;
            printk("jarrett set dload mode to blow efuse,dload_mode_addr:\n");
      }
      printk("set efuse mode complete\n");
      is_efuse_mode = true;
      msm_restart(0,NULL);
      is_efuse_mode = false;
}
EXPORT_SYMBOL(set_efuse_mode);
#endif
...
extern  void bq24196_otg_poweroff_stop(int on);
static void __msm_power_off(int lower_pshold)
{
	...
	set_dload_mode(0);
#endif
	printk(KERN_CRIT "close otg when power off\n");

	bq24196_otg_poweroff_stop(0);
