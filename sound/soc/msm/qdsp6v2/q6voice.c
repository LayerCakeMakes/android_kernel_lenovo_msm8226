...
int voc_end_voice_call(uint32_t session_id)
{
	struct voice_data *v = voice_get_session(session_id);
	int ret = 0;
	extern void msm_q6_voice_call_session(int active);

	printk(KERN_DEBUG"voice call is end, ssid:%d\n", session_id);
	if (v == NULL) {
		pr_err("%s: invalid session_id 0x%x\n", __func__, session_id);
...
int voc_start_voice_call(uint32_t session_id)
{
	...
	printk(KERN_DEBUG"start voice call, ssid:%d\n", session_id);
	if (v == NULL) {
		pr_err("%s: invalid session_id 0x%x\n", __func__, session_id);
