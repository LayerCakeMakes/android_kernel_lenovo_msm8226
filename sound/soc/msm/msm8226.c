...
#define GPIO_TER_MI2S_SCK   49
#define GPIO_TER_MI2S_WS    50 
#define GPIO_TER_MI2S_DATA0 51
#define GPIO_TER_MI2S_DATA1 52 
...
static struct request_gpio ter_mi2s_gpio[] = {

	{
		.gpio_no = GPIO_TER_MI2S_SCK,
		.gpio_name = "TER_MI2S_SCK",
	},
	{
		.gpio_no = GPIO_TER_MI2S_WS,
		.gpio_name = "TER_MI2S_WS",
	},

	{
		.gpio_no = GPIO_TER_MI2S_DATA0,
		.gpio_name = "TER_MI2S_DATA0",
	},
	{
		.gpio_no = GPIO_TER_MI2S_DATA1,
		.gpio_name = "TER_MI2S_DATA1",
	},
...
static struct mi2s_clk ter_mi2s_clk;
...
static atomic_t gpio_requested;

static int msm8226_ter_mi2s_free_gpios(void)
{
	int	i;

	if (atomic_dec_return(&gpio_requested) == 0) {
		for (i = 0; i < ARRAY_SIZE(ter_mi2s_gpio); i++)
                	gpio_free(ter_mi2s_gpio[i].gpio_no);
			}
	return 0;
}
...
static void msm8226_mi2s_shutdown(struct snd_pcm_substream *substream)
{
	...
	if (atomic_dec_return(&ter_mi2s_clk.mi2s_rsc_ref) == 0) {
		...
		msm8226_ter_mi2s_free_gpios();
	}
}

static int msm8226_configure_ter_mi2s_gpio(void)
{
	int	rtn;
	int	i;

	if (atomic_inc_return(&gpio_requested) == 1) {
		for (i = 0; i < ARRAY_SIZE(ter_mi2s_gpio); i++) {

			rtn = gpio_request(ter_mi2s_gpio[i].gpio_no,
					ter_mi2s_gpio[i].gpio_name);

			pr_info("%s: gpio = %d, gpio name = %s, rtn = %d\n", __func__,
			ter_mi2s_gpio[i].gpio_no, ter_mi2s_gpio[i].gpio_name, rtn);		
			if (rtn) {
				pr_err("%s: Failed to request gpio %d\n",
					   __func__,
					   ter_mi2s_gpio[i].gpio_no);
				break;
			}
		}
	}
...
static int msm8226_mi2s_startup(struct snd_pcm_substream *substream)
{
	...
	if (atomic_inc_return(&ter_mi2s_clk.mi2s_rsc_ref) == 1) {
		pr_info("%s: acquire mi2s resources\n", __func__);
		msm8226_configure_ter_mi2s_gpio();	
...
int msm_q6_control_ter_mi2s_clocks(int en)
{
	int ret = 0;
	int twice = 1;
...
EXPORT_SYMBOL(msm_q6_control_ter_mi2s_clocks);

static int msm_snd_mi2s_params(struct snd_pcm_substream *substream,
			     struct snd_pcm_hw_params *params)
{
	pr_info("configure mi2s params\n");
	return 0;
}
