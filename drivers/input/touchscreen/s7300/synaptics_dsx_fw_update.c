...
#define FW_IMAGE_GIS 	"PR1566696_gis.img"
#define FW_IMAGE_OFILM "PR1566696_ofilm.img"
...
static int fwu_start_reflash(void)
{
	...
	if (fwu->ext_data_source)
		...
		retval = fwu->fn_ptr->read(fwu->rmi4_data,
				fwu->rmi4_data->f01_query_base_addr,
				product_info,
				4);
		if (retval < 0) {
			dev_dbg(&fwu->rmi4_data->i2c_client->dev,
					"%s: Failed to read product_info from device\n",
					__func__);
		}

		dev_info(&fwu->rmi4_data->i2c_client->dev,
				"%s: Product_info: 0x%x, 0x%x, 0x%x, 0x%x \n",
				__func__, product_info[0], product_info[1],
				product_info[2], product_info[3]);

			switch(product_info[2]){
				case VENDOR_GIS:
					snprintf(fwu->firmware_name, NAME_BUFFER_SIZE, "%s", FW_IMAGE_GIS);
					break;
				case VENDOR_OFILM:
					snprintf(fwu->firmware_name, NAME_BUFFER_SIZE, "%s", FW_IMAGE_OFILM);
					break;
				default:
					dev_err(&fwu->rmi4_data->i2c_client->dev,
							"%s: Un-support vendor id[%x].\n",
							__func__, product_info[2]);
					retval = -EINVAL;
					goto exit;
