...
#include "mdss_dsi.h"
#include "mdss_fb.h"
#include "mdss_panel_mode.h"


#define DT_CMD_HDR 6
#define MAX_LCD_NAME 16

DEFINE_LED_TRIGGER(bl_led_trigger);

static cabc_mode_t cabc_mode = CABC_ON;

#define AUO_PANEL 0
#define CPT_PANEL 1
#define AUO_PANEL_NAME "AUO_OTC3108B"
#define CPT_PANEL_NAME "CPT_NT71410"

static char lcd_id = AUO_PANEL;


static char lcd_name[MAX_LCD_NAME] = AUO_PANEL_NAME;

extern int g_board_id;
static int set_cabc_mode(struct mdss_dsi_ctrl_pdata *ctrl, int mode);
...
static void mdss_dsi_panel_bklt_pwm(struct mdss_dsi_ctrl_pdata *ctrl, int level)
{
	...
	if (ctrl->panel_data.panel_info.pwm_config_required)
	{
		ctrl->panel_data.panel_info.pwm_config_required = 0;
		gpio_set_value(ctrl->pwm_pmic_gpio, 0);
		if (g_board_id < 1650 ) {
			/*non pvt board */
			cabc_mode = CABC_OFF;
		}
		if (cabc_mode == CABC_OFF)
		set_cabc_mode(ctrl, CABC_OFF);
	}
	if (ret)
...
void mdss_dsi_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	...
	if (enable) {
		...
		if (CPT_PANEL == lcd_id) msleep(100);
	} else {
...
extern int touchscreen_enable_from_lcd(int val);
#endif
...
#ifdef CONFIG_TOUCHSCREEN_SYNAPTICS_DSX_I2C
	if(touchscreen_enable_from_lcd(1))
		pr_err("%s: disable touchscreen fail\n", __func__);
#endif
...
#ifdef CONFIG_TOUCHSCREEN_SYNAPTICS_DSX_I2C
	if(touchscreen_enable_from_lcd(0))
		pr_err("%s: enable touchscreen fail\n", __func__);
#endif
			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->cabc_on_cmds,
		"qcom,mdss-dsi-cabc-on-command", NULL);

	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->cabc_off_cmds,
		"qcom,mdss-dsi-cabc-off-command", NULL);

	pr_info(" on_cmds cnt %d, %d\n", ctrl_pdata->on_cmds.cmd_cnt, ctrl_pdata->cabc_on_cmds.cmd_cnt);
	return 0;
	...
	bool cmd_cfg_cont_splash)
{
...
#if 0
		strlcpy(lcd_name, panel_name, MAX_LCD_NAME<strlen(panel_name)?MAX_LCD_NAME:strlen(panel_name));
		lcd_name[MAX_LCD_NAME-1] = '\0';
		p = strchr(lcd_name,' ');
		if (p){
			*p = '\0';
		}
#endif
		if (strstr(panel_name, "nt714"))
		{
			lcd_id = CPT_PANEL;
			strcpy(lcd_name, CPT_PANEL_NAME);
		}
	}
	...
	if (!cont_splash_enabled) {
		...
		ctrl_pdata->panel_data.panel_info.pwm_config_required  = 0;
	} else {
		pr_info("%s:%d Continuous splash flag enabled.\n",
				__func__, __LINE__);

		ctrl_pdata->panel_data.panel_info.cont_splash_enabled = 1;
		ctrl_pdata->panel_data.panel_info.pwm_config_required = 1;
	}
...
#if 0
static char cabc_set[2] = {0x15, 0x55};  /* DTYPE_DCS_WRITE1 */
static struct dsi_cmd_desc cabc_mode_cmd= {
        {DTYPE_GEN_WRITE2, 1, 0, 0, 1, sizeof(cabc_set)},
       cabc_set 
};
#endif
static int set_cabc_mode(struct mdss_dsi_ctrl_pdata *ctrl, int mode)
{
	...
	if (1)  {
        /* auo panel */
                pr_info("cabc %x, cabc %d, now %d \n",DTYPE_GEN_WRITE2, mode, cabc_mode);
                memset(&cmdreq, 0, sizeof(cmdreq));
                cmdreq.flags = CMD_REQ_COMMIT | CMD_CLK_CTRL;
                cmdreq.rlen = 0;
                cmdreq.cb = NULL;
                ...
                switch(mode){
                        case 0:
                                //cabc_set[1] = 0xAA;
                                //cmdreq.cmds = &cabc_mode_cmd;
                                cmdreq.cmds_cnt = ctrl->cabc_on_cmds.cmd_cnt;
				if (cmdreq.cmds_cnt == 0 )
					break;
                                cmdreq.cmds = ctrl->cabc_off_cmds.cmds;
                                mdss_dsi_cmdlist_put(ctrl, &cmdreq);
                                cabc_mode = CABC_OFF;
				...
				if (cmdreq.cmds_cnt == 0 )
					break;
                                cmdreq.cmds = ctrl->cabc_on_cmds.cmds;
                                mdss_dsi_cmdlist_put(ctrl, &cmdreq);
                                cabc_mode = CABC_ON;
                                break;
                }
...
char *mdss_get_lcd_name(void)
{
	return lcd_name;
}
int mdss_get_cabc_mode(void)
{
	return cabc_mode;
}

int mdss_set_cabc_mode(struct msm_fb_data_type *mfd, int mode)
{
        struct mdss_dsi_ctrl_pdata *ctrl = NULL;
