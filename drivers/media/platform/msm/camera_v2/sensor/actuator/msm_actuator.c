	...
	struct msm_actuator_move_params_t *move_params)
{
	...
	if (copy_from_user(&ringing_params_kernel,
        ...
        /*
	printk("%s:dest_step_pos=%d,dir=%d,num_steps=%d,cur_lens_pos=%d\n",
		__func__,dest_step_pos, dir, num_steps,
...
//lenovo added
		if (lenovo_act_debug){
			printk("debug mode enabled, return directly\n");
			break;
		}
//lenovo ended
...
//lenovo added
static void msm_act_init_debugfs(void* ptr);
//static void msm_act_clean_debugfs(void);
//lenovo ended
...
//lenovo added
	msm_act_init_debugfs((void*)msm_actuator_t);
//lenovo ended
	CDBG("Exit\n");
	return rc;
}
...
//lenovo added debugfs
static struct msm_actuator_ctrl_t *s_ctrl = NULL;

static  ssize_t lens_pos_debug_read(struct file *file,
        char __user *buf,size_t count, loff_t *pos)
{
	...
	rc = s_ctrl->i2c_client.i2c_func_tbl->i2c_read(
                &s_ctrl->i2c_client, 0xc4,&lens_pos,MSM_CAMERA_I2C_WORD_DATA);
        if (rc < 0) {
                pr_err("i2c write error:%d\n", rc);
        }
	printk("YAO,%s,reg 000=%x\n",__func__,lens_pos);

	lens_pos = lens_pos & 0x3ff;
...
static ssize_t lens_pos_debug_write(struct file *filp,
        const char __user *ubuf, size_t cnt, loff_t *ppos)
{
	...
	struct msm_camera_i2c_reg_setting reg_setting;
        ...
        lens_pos  = simple_strtol(lbuf,NULL,0);

	printk("YAO,%s,lens_pos=%d\n",__func__,lens_pos);

	s_ctrl->i2c_reg_tbl[0].reg_addr = 0xc4 | ((lens_pos >> 8)& 0x3);
	s_ctrl->i2c_reg_tbl[0].reg_data = lens_pos & 0xff;
	s_ctrl->i2c_reg_tbl[0].delay = 0;

	reg_setting.reg_setting = s_ctrl->i2c_reg_tbl;
	reg_setting.data_type = s_ctrl->i2c_data_type;
        reg_setting.size = 1;

	printk("YAO, reg=%x, data=%x\n",reg_setting.reg_setting[0].reg_addr,
		reg_setting.reg_setting[0].reg_data);

        rc = s_ctrl->i2c_client.i2c_func_tbl->i2c_write_table_w_microdelay(
                &s_ctrl->i2c_client, &reg_setting);
        if (rc < 0) {
                pr_err("i2c write error:%d\n", rc);
                return rc;
        }
...
static const struct file_operations lens_pos_debug_ops = {
        .read = lens_pos_debug_read,
        .write = lens_pos_debug_write,
};

static int  i2c_type = 1; //1 for data type byte, 2 for data type word
static uint32_t i2c_addr = 0;


static  ssize_t i2c_data_debug_read(struct file *file,
        char __user *buf,size_t count, loff_t *pos)
{
        const int size = 32;
        char buffer[size];
	...
	rc = s_ctrl->i2c_client.i2c_func_tbl->i2c_read(
                &s_ctrl->i2c_client,i2c_addr,&data,i2c_type);
        if (rc < 0) {
                pr_err("i2c write error:%d\n", rc);
        }
        printk("YAO,%s,reg 004=%x\n",__func__,data);
...
static ssize_t i2c_data_debug_write(struct file *filp,
        const char __user *ubuf, size_t cnt, loff_t *ppos)
{
        const int size = 32;
        char lbuf[size];
	...
	uint32_t data;
        ...
        data = simple_strtol(lbuf,NULL,0);
...
static const struct file_operations i2c_data_debug_ops = {
        .read = i2c_data_debug_read,
        .write = i2c_data_debug_write,
};


static  ssize_t i2c_addr_debug_read(struct file *file,
        char __user *buf,size_t count, loff_t *pos)
{
        const int size = 32;
        char buffer[size];
        ...
        n = scnprintf(buffer,size,"i2c_addr = %x\n",i2c_addr);
        buffer[n] = 0;

        return simple_read_from_buffer(buf,count,pos,buffer,n);
}

static ssize_t i2c_addr_debug_write(struct file *filp,
        const char __user *ubuf, size_t cnt, loff_t *ppos)
{
        const int size = 32;
        char lbuf[size];
        ...
        i2c_addr = simple_strtol(lbuf,NULL,0);
        return cnt;
}

static const struct file_operations i2c_addr_debug_ops = {
        .read = i2c_addr_debug_read,
        .write = i2c_addr_debug_write,
};


static  ssize_t i2c_type_debug_read(struct file *file,
        char __user *buf,size_t count, loff_t *pos)
{
        const int size = 32;
        char buffer[size];
        ...
        n = scnprintf(buffer,size,"i2c_type = %d (1 for byte data,2 for word data\n",i2c_type);
        buffer[n] = 0;

        return simple_read_from_buffer(buf,count,pos,buffer,n);
}

static ssize_t i2c_type_debug_write(struct file *filp,
        const char __user *ubuf, size_t cnt, loff_t *ppos)
{
        const int size = 32;
        char lbuf[size];
        ...
        i2c_type = simple_strtol(lbuf,NULL,0);
        return cnt;
}

static const struct file_operations i2c_type_debug_ops = {
        .read = i2c_type_debug_read,
        .write = i2c_type_debug_write,
};
        ...
        char __user *buf,size_t count, loff_t *pos)
{
        ...
        n = scnprintf(buffer,size,"actuator debug mode = %d\n",lenovo_act_debug);
        buffer[n] = 0;
        ...
        const char __user *ubuf, size_t cnt, loff_t *ppos)
{
        ...
        lenovo_act_debug = simple_strtol(lbuf,NULL,0);
...
static struct dentry* msm_act_dir = NULL;
static struct dentry* lens_pos_dir = NULL;
static struct dentry* debug_mode_dir=NULL;
static struct dentry* i2c_type_dir=NULL;
static struct dentry* i2c_addr_dir=NULL;
static struct dentry* i2c_data_dir=NULL;
//static struct dentry* ae_disable_dir=NULL;


static void msm_act_init_debugfs(void* ptr)
{
	s_ctrl = (struct msm_actuator_ctrl_t*)(ptr);

        msm_act_dir = debugfs_create_dir("msm_vcm_act",NULL);

        lens_pos_dir = debugfs_create_file("lens_pos",S_IFREG | S_IRUGO| S_IWUSR,
                msm_act_dir,NULL,&lens_pos_debug_ops);

        debug_mode_dir =debugfs_create_file("debug_mode",S_IFREG | S_IRUGO| S_IWUSR,
                msm_act_dir,NULL,&debug_mode_debug_ops);

        i2c_type_dir =debugfs_create_file("i2c_type",S_IFREG | S_IRUGO| S_IWUSR,
                msm_act_dir,NULL,&i2c_type_debug_ops);

        i2c_addr_dir =debugfs_create_file("i2c_addr",S_IFREG | S_IRUGO| S_IWUSR,
                msm_act_dir,NULL,&i2c_addr_debug_ops);

	i2c_data_dir =debugfs_create_file("i2c_data",S_IFREG | S_IRUGO| S_IWUSR,
                msm_act_dir,NULL,&i2c_data_debug_ops);
...
static void msm_act_clean_debugfs(void)
{
        ...
        debugfs_remove(lens_pos_dir);
        debugfs_remove(msm_act_dir);
}*/
